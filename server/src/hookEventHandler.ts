...
export class HookEventHandler {
  ...
  /** Map of provider id → provider, for routing events to the correct normalizer. */
  private readonly providers: Map<string, HookProvider>;
  /** Default provider: used when no providerId match is found (backward compat). */
  private readonly defaultProvider: HookProvider;
  ...
  /** Resolve a provider by id, falling back to the default provider. */
   ...
   *  Uses the default provider — only Claude has team support today. */
  private getSubagentToolSet(): ReadonlySet<string> {
    if (this.defaultProvider.team) {
      return new Set<string>([
        ...this.defaultProvider.team.teammateSpawnTools,
        ...this.defaultProvider.team.withinTurnSubagentTools,
      ]);
    }
    return this.defaultProvider.subagentToolNames;
  }
  ...
  handleEvent(providerId: string, event: HookEvent): void {
    ...
    // Each provider translates its own raw event format into the canonical AgentEvent.
    // All provider-specific fields (tool_name, tool_input, agent_type, etc.) are
    // extracted by the provider's normalizeHookEvent. Downstream dispatch uses the
    ...
    switch (normEvent.kind) {
      ...
      case 'subagentTurnEnd':
        ...
        if (!this.defaultProvider.team) return;
  ...
  private handleSubagentStart(event: HookEvent, agent: AgentState, agentId: number): void {
    const agentType = this.defaultProvider.team?.extractTeammateNameFromEvent(event) ?? 'unknown';
    ...
    if (this.defaultProvider.team && agent.currentHookIsTeammateSpawn === true && agent.teamName) {
      if (debug)
        console.log(
  ...
  private handleTeammateIdle(event: HookEvent, agent: AgentState, agentId: number): void {
    const teammateName = this.defaultProvider.team?.extractTeammateNameFromEvent(event);
    const inlineTeammates = getInlineTeammates(agentId, this.agents);
